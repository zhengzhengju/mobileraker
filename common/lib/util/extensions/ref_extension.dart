...
extension MobilerakerAutoDispose on AutoDisposeRef {
  ...
  Stream<T> watchAsSubject<T>(ProviderListenable<AsyncValue<T>> provider,
    ...
    listen<AsyncValue<T>>(provider, (prev, next) {
      if (next.isRefreshing) {
        logger.error('isRefreshing SKIPPED');
        return;
      }
      next.when(
  ...
  Future<T> watchWhere<T>(ProviderListenable<AsyncValue<T>> provider, bool Function(T) evaluatePredicate,
    ...
    onDispose(() {
      if (!completer.isCompleted && throwIfDisposeBeforeComplete) {
        logger.error('Provider with ref $this diposed before `where` could complete');
          ...
          data: (d) {
            if (evaluatePredicate(d)) {
              if (completer.isCompleted) {
                logger.info('watchWhere just forces owner to invalidate! Ref:$this');
                invalidateSelf();
              } else {
              ...
              if (completer.isCompleted) {
                if (kDebugMode) logger.error('THIS IS NEW 11 $this');
                invalidateSelf();
              }
            }
          },
  ...
  Future<T> watchWhereNotNull<T>(ProviderListenable<AsyncValue<T?>> provider) {
    ...
    onDispose(() {
      if (!completer.isCompleted) {
        logger.error('Provider with ref $this diposed before `whereNotNull` could complete');
      ...
      next.when(
          data: (d) {
            if (d != null) {
              ...
              if (completer.isCompleted) {
                if (kDebugMode) logger.error('THIS IS NEW NULL');
                invalidateSelf();
              }
            }
          },
  ...
  Future<T> readWhere<T>(ProviderListenable<AsyncValue<T>> provider, bool Function(T) evaluatePredicate,
    ...
    onDispose(() {
      if (!completer.isCompleted && throwIfDisposeBeforeComplete) {
        logger.error('Provider with ref $this diposed before `read` could complete');
        completer.completeError(
