...
class ObicoTunnelService {
  ...
  Future<Uri> linkApp({String? printerId}) async {
    ...
    try {
      ...
      logger.info('Obico Linking Result: $resultParameters');

      return _parseAndValidateTunnelUri(resultParameters);
    } on PlatformException catch (e) {
      logger.error('Error during Obico Setup', e);
  ...
  Future<PlatformInfo> retrievePlatformInfo(Uri tunnelUri) async {
    ...
    logger.info('Received platform info from obico tunnel: ${response.data}');
    try {
      return PlatformInfo.fromJson(response.data);
    } catch (e, s) {
      logger.info('Error while parsing PlatformInfo response from obico tunnel: ${response.data}', e, s);
  ...
  Uri _parseAndValidateTunnelUri(Map<String, String> queryParameters) {
    ...
    if (endpoint == null) {
      logger.info('Obico linking failed, did not receive tunnel_endpoint');
    ...
    if (userInfo.length != 2) {
      logger.info('Obico linking failed, did not receive username and password. UserInfo length: ${userInfo.length}');
      throw const ObicoException('Obico linking failed');
    }
    return tunnelUri;
  }
