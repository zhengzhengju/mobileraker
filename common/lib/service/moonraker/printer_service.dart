...
class PrinterService {
  PrinterService(AutoDisposeRef ref, this.ownerUUID)
    ...
    ref.listen(klipperProvider(ownerUUID).selectAs((value) => value.klippyState), (previous, next) {
      logger.info('Printer Service received klippyState: ${next.valueOrNull}');
  ...
  Future<void> refreshPrinter() async {
    try {
      ...
      _removeJrpcHandlers();
      logger.info('Refreshing printer for uuid: $ownerUUID');
      ...
      logger.error('Unable to refresh Printer $ownerUUID...', e, s);
      ...
      logger.error('Unexpected exception thrown during refresh $ownerUUID...', e, s);
      _showExceptionSnackbar(e, s);
      _printerStreamCtler.addError(e, s);
      if (e is Future) {
        e.then((value) => logger.error('Error was a Future: Data. $value'),
            onError: (e, s) => logger.error('Error was a Future: Error. $e', e, s));
      }
  ...
  Future<bool> gCode(String script, {bool throwOnError = false, bool showSnackOnErr = true}) async {
    try {
      ...
      logger.info('GCode "$script" executed successfully!');
      return true;
    } on JRpcError catch (e, s) {
      var gCodeException = GCodeException.fromJrpcError(e, parentStack: s);
      logger.info('GCode execution failed: ${gCodeException.message}');
  ...
  Future<List<ConsoleEntry>> gcodeStore() async {
    logger.info('Fetching cached GCode commands');
    try {
      ...
      logger.info('Received cached GCode commands');
      return List.generate(raw.length, (index) => ConsoleEntry.fromJson(raw[index]));
    } on JRpcError catch (e) {
      logger.error('Error while fetching cached GCode commands: $e');
    }
    return List.empty();
  }

  Future<List<Command>> gcodeHelp() async {
    logger.info('Fetching available GCode commands');
    try {
      ...
      logger.info('Received ${raw.length} available GCode commands');
  ...
  Future<void> updateCurrentFile(String? file) async {
    logger.info('Also requesting an update for current_file: $file');

    try {
      var gCodeMeta = (file?.isNotEmpty == true) ? await _fileService.getGCodeMetadata(file!) : null;

      if (hasCurrent) {
        logger.info('UPDATED current_file: $gCodeMeta');
        current = current.copyWith(currentFile: gCodeMeta);
      }
    } catch (e, s) {
      logger.error('Error while updating current_file', e, s);
  ...
  Future<void> _temperatureStore(PrinterBuilder printer) async {
    if (disposed) return;
    logger.info('Fetching cached temperature store data');
    ...
    try {
      ...
      logger.info('Received cached temperature store for $sensors');

      raw.forEach((key, value) {
        _parseObjectType(key, raw, printer);
      });
    } on JRpcError catch (e) {
      logger.error('Error while fetching cached temperature store: $e');
    }
  }

  Future<PrinterBuilder> _printerObjectsList() async {
    // printerStream.value = Printer();
    logger.info('>>>Querying printers object list');
  ...
  _onStatusUpdateHandler(Map<String, dynamic> rawMessage) {
    ...
    if (!hasCurrent) {
      logger.warning('Received statusUpdate before a printer was parsed initially!');
      return;
    }
  ...
  _parseObjectType(String key, Map<String, dynamic> json, PrinterBuilder printer) {
    ...
    try {
      ...
      logger.error('Error while parsing $key object', e, s);
      _printerStreamCtler.addError(e, s);
  ...
  _parsePrinterObjectsList(Map<String, dynamic> result) {
    logger.info('<<<Received printer objects list!');
    logger.verbose('PrinterObjList: ${const JsonEncoder.withIndent('  ').convert(result)}');
  ...
  _parseQueriedObjects(dynamic response, PrinterBuilder printer) async {
    logger.info('<<<Received queried printer objects');
    logger.verbose('PrinterObjectsQuery: ${const JsonEncoder.withIndent('  ').convert(response)}');
  ...
  _printerObjectsQuery(PrinterBuilder printer) async {
    if (disposed) return;
    logger.info('>>>Querying Printer Objects!');
  ...
  _makeSubscribeRequest(List<String> queryableObjects) {
    logger.info('Subscribing printer objects for ws-updates!');
