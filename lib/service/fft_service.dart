...
class FftService {
  ...
  start() async {
    ...
    logger.info('Initialized capture reported init-state: $res');

    _capture.start(
      (e) {
        // logger.info('Received data from capturer: ${e.runtimeType}: ${e.length}');
        MicData micData;
        if (e is MicData) {
          micData = e;
        } else {
        ...
        // logger.info('Actual sample rate: $actualSampleRate');
        if (_sendPort == null) return;
        if (actualSampleRate == null) {
          logger.warning('Sample rate is null, try again on next chunk');
          return;
        }
        _sendPort!.send(_MicDataMessage(micData, actualSampleRate));
      },
      (e) {
        logger.warning('Error in capturer', e);
      },
  ...
  _onIsolateMessage(dynamic message) {
    ...
    if (message is SendPort) {
      ...
      // logger.info('Received peak frequency from isolate: ${message.frequency}');
  ...
  void dispose() {
    ...
    _receivePort.close();
    _isolate?.kill();
    logger.info('Stopped FFT service, killed isolate');
  }
}
...
class _FFTIsolate {
  ...
  void _onUiMessage(dynamic message) {
    // logger.info('Received message from UI: $message');
